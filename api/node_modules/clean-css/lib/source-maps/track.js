...
function snapshot(data, context, fallbacks) {
  ...
  var sourceMetadata = context.sourceMapTracker.isTracking(metadata.source)
    ? context.sourceMapTracker.originalPositionFor(
        metadata,
        data,
        fallbacks || 0
      )
  ...
  if (context.sourceMapInlineSources) {
    var sourceMapSourcesContent = context.sourceMapTracker.sourcesContentFor(
      context.source
    );
    sourceContent =
      sourceMapSourcesContent && sourceMapSourcesContent[metadata.source]
...
function trackSuffix(data, context) {
  ...
  for (var i = 0, l = parts.length; i < l; i++) {
    ...
    while (true) {
      ...
      var encodedValues = escaped
        .substring(escaped.indexOf("(") + 1, escaped.indexOf(")"))
        .split(",");
      context.line += ~~encodedValues[0];
      context.column =
        (~~encodedValues[0] === 0 ? context.column : 0) + ~~encodedValues[1];
