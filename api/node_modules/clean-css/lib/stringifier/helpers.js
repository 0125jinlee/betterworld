...
function afterClosingBrace(token, valueIndex) {
  return (
    token[valueIndex][0][token[valueIndex][0].length - 1] == ")" ||
    token[valueIndex][0].indexOf("__ESCAPED_URL_CLEAN_CSS") === 0
  );
}
...
function value(tokens, position, isLast, context) {
  ...
  for (var j = 1, m = token.length; j < m; j++) {
    ...
    if (
      j < m - 1 &&
      (inFilter(token) || !inSpecialContext(token, j, context))
    ) {
      store(" ", context);
    } else if (
      j == m - 1 &&
      !isLast &&
      hasMoreProperties(tokens, position + 1)
    ) {
