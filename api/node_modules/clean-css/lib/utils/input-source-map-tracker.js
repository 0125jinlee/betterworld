...
function fromSource(self, data, whenDone, context) {
  ...
  while (context.cursor < data.length) {
    ...
    if (nextAt == markerStartMatch.index) {
      ...
      var isRemote =
        /^https?:\/\//.test(sourceMapFile) || /^\/\//.test(sourceMapFile);
      ...
      if (isRemote) {
        ...
        if (isDataUri) {
          ...
          sourceMapPath = path.resolve(
            self.options.root,
            path.join(sourceDir || "", sourceMapFile)
          );
...
function fetchMapFile(self, sourceUrl, context, done) {
  fetch(
    ...
    function(data) {
      self.trackLoaded(
        context.files[context.files.length - 1] || undefined,
        sourceUrl,
        data
      );
      done();
    ...
    function(message) {
      context.errors.push(
        'Broken source map at "' + sourceUrl + '" - ' + message
      );
      return done();
    }
  );
...
function trackContentSources(self, sourceFile) {
  ...
  consumer.sources.forEach(function(file, index) {
    ...
    sourcesMapping[uniquePath] =
      consumer.sourcesContent && consumer.sourcesContent[index];
  });
...
function _resolveSources(self, remaining, whenDone) {
  ...
  if (isRemote && self.localOnly) {
    self.warnings.push(
      'No callback given to `#minify` method, cannot fetch a remote file from "' +
        originalFile +
        '"'
    );
    return processNext();
  ...
  if (isRemote) {
    fetch(
      ...
      function(message) {
        self.warnings.push(
          'Broken original source file at "' + originalFile + '" - ' + message
        );
        processNext();
      }
    );
    ...
    if (fs.existsSync(fullPath))
      self.sourcesContent[sourceFile][originalFile] = fs.readFileSync(
        fullPath,
        "utf-8"
      );
    else
...
InputSourceMapStore.prototype.trackLoaded = function(
  sourcePath,
  mapPath,
  mapData
) {
  var relativeTo = this.options.explicitTarget
    ? this.options.target
    : this.options.root;
...
InputSourceMapStore.prototype.originalPositionFor = function(
  sourceInfo,
  token,
  allowNFallbacks
) {
  return originalPositionIn(
    this.maps[sourceInfo.source],
    sourceInfo.line,
    sourceInfo.column,
    token,
    allowNFallbacks
