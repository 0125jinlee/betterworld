...
function reduceNonAdjacent(tokens, options, context) {
  ...
  for (var i = tokens.length - 1; i >= 0; i--) {
    ...
    var isComplexAndNotSpecial =
      token[1].length > 1 && !isSpecial(options, selectorAsString);
    var wrappedSelectors = options.sourceMap
      ? wrappedSelectorsFrom(token[1])
      : token[1];
...
function reduceComplexNonAdjacentCases(tokens, candidates, options, context) {
  ...
  allSelectors: for (var complexSelector in candidates) {
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      if (
        stringifyBody(reducedBodies[reducedBodies.length - 1]) !=
        stringifyBody(reducedBodies[0])
      )
...
function reduceSelector(
  ...
  for (j = 0, m = bodiesAsList.length; j < m; j++) {
    if (bodiesAsList[j].length > 0)
      joinsAt.push(
        (joinsAt.length > 0 ? joinsAt[joinsAt.length - 1] : 0) +
          bodiesAsList[j].length
      );
  }
