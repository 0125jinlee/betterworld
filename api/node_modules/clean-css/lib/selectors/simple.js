...
function isNegative(property, idx) {
  return (
    property.value[idx] &&
    property.value[idx][0][0] == "-" &&
    parseFloat(property.value[idx][0]) < 0
  );
}
...
function precisionMinifier(_, value, precisionOptions) {
  ...
  return value
    .replace(precisionOptions.regexp, function(match, number) {
      return (
        Math.round(parseFloat(number) * precisionOptions.multiplier) /
          precisionOptions.multiplier +
        "px"
      );
    })
    .replace(/(\d)\.($|\D)/g, "$1$2");
...
function multipleZerosMinifier(property) {
  ...
  if (
    ...
    if (property.name.indexOf("box-shadow") > -1) spliceAt = 2;
    else spliceAt = 1;
  }
...
function minifyBorderRadius(property) {
  ...
  if (values.length == 3 && values[1][0] == "/" && values[0][0] == values[2][0])
    ...
    values[1][0] == values[6][0] &&
    values[2][0] == values[7][0] &&
    values[3][0] == values[8][0]
  )
    spliceAt = 4;
...
function minifyFilter(property) {
  if (property.value.length == 1) {
    property.value[0][0] = property.value[0][0].replace(
...
function minifyFont(property) {
  var values = property.value;
  var hasNumeral =
    FONT_NUMERAL_WEIGHTS.indexOf(values[0][0]) > -1 ||
  ...
  if (FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[0][0]) > -1)
    ...
    values[1] &&
    FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[1][0]) > -1
  )
    toOptimize = 1;
  else if (
    values[2] &&
    FONT_NAME_WEIGHTS_WITHOUT_NORMAL.indexOf(values[2][0]) > -1
  )
    toOptimize = 2;
  else if (FONT_NAME_WEIGHTS.indexOf(values[0][0]) > -1) toOptimize = 0;
  ...
  if (toOptimize !== undefined) {
    property.value[toOptimize][0] = valueMinifiers["font-weight"](
      values[toOptimize][0]
    );
    property.dirty = true;
  }
...
function optimizeBody(properties, options) {
  ...
  for (var i = 0, l = _properties.length; i < l; i++) {
    ...
    if (
      property.hack &&
      (((property.hack == "star" || property.hack == "underscore") &&
        !options.compatibility.properties.iePrefixHack) ||
        (property.hack == "backslash" &&
    ...
    if (
      ...
      (isNegative(property, 0) ||
        isNegative(property, 1) ||
        isNegative(property, 2) ||
        isNegative(property, 3))
    )
      property.unused = true;
    ...
    else if (name == "filter") minifyFilter(property);
    else if (name == "font") minifyFont(property);
  }
...
function cleanupCharsets(tokens) {
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (hasCharset || token[1][0].indexOf(CHARSET_TOKEN) == -1) {
      ...
      tokens.unshift([
        "at-rule",
        [token[1][0].replace(CHARSET_REGEXP, CHARSET_TOKEN)]
      ]);
    }
  }
}
...
function buildPrecision(options) {
  ...
  precision.value =
    options.roundingPrecision === undefined
      ? DEFAULT_ROUNDING_PRECISION
      : options.roundingPrecision;
  precision.multiplier = Math.pow(10, precision.value);
  precision.regexp = new RegExp(
    "(\\d*\\.\\d{" + (precision.value + 1) + ",})px",
    "g"
  );
  ...
  for (var i = 0, l = tokens.length; i < l; i++) {
    ...
    if (token[0] == "at-rule" && IMPORT_REGEXP.test(token[1]) && afterContent) {
      context.warnings.push(
        'Ignoring @import rule "' +
          token[1] +
          '" as it appears after rules thus browsers will ignore them.'
      );
      token[1] = "";
    }
