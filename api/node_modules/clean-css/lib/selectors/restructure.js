...
function restructure(tokens, options) {
    ...
    for (var i = tokensVariant.length - 1; i >= 0; i--) {
      allSelectorsSize +=
        tokensVariant[i][2].length > propertiesCount
          ? stringifySelectors(tokensVariant[i][1]).length
          : -1;
    }
      ...
      for (j = mergeableToken[2].length - 1; j >= 0; j--) {
        ...
        for (k = 0, m = properties.length; k < m; k++) {
          ...
          if (
            ...
            stringifyBody([mergeableProperty]) == propertyBody
          ) {
    ...
    for (j = properties.length - 1; j >= 0; j--) {
      ...
      for (k = 0; k < movedCount; k++) {
        ...
        if (
          ...
          !canReorderSingle(property, movedProperty) &&
  ...
  var position =
    tokens[0] &&
    tokens[0][0] == "at-rule" &&
    tokens[0][1][0].indexOf("@charset") === 0
      ? 1
      : 0;
  for (; position < tokens.length - 1; position++) {
    var isImportRule =
      tokens[position][0] === "at-rule" &&
      tokens[position][1][0].indexOf("@import") === 0;
    var isEscapedCommentSpecial =
      tokens[position][0] === "text" &&
      tokens[position][1][0].indexOf("__ESCAPED_COMMENT_SPECIAL") === 0;
