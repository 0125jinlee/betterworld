...
function importFrom(data, context) {
  ...
  for (; nextEnd < data.length; ) {
    ...
    context.left.unshift([
      data.substring(nextEnd + 1),
      override(context, { shallow: false })
    ]);
...
function nextImportAt(data, cursor) {
  ...
  if (nextLowerCase > -1 && nextUpperCase == -1) return nextLowerCase;
  else if (nextLowerCase == -1 && nextUpperCase > -1) return nextUpperCase;
  else return Math.min(nextLowerCase, nextUpperCase);
}
...
function inline(data, nextStart, nextEnd, context) {
  ...
  var isQuoted = /^['"]/.exec(
    importDeclaration.substring(urlStartsAt, urlStartsAt + 2)
  );
  ...
  if (
    isRemote &&
    (context.localOnly || !allowedResource(importedFile, true, context.imports))
  ) {
    ...
    else restoreImport(importedFile, mediaQuery, context);
    ...
    if (context.afterImport)
      context.warnings.push(
        'Ignoring local @import of "' +
          importedFile +
          '" as after other inlined content.'
      );
    else restoreImport(importedFile, mediaQuery, context);
    return processNext(context);
  }
...
function allowedResource(importedFile, isRemote, rules) {
  ...
  for (var i = 0; i < rules.length; i++) {
    ...
    if (rule == "all") allowed = true;
    else if (isRemote && rule == "local") allowed = false;
    else if (isRemote && rule == "remote") allowed = true;
    else if (!isRemote && rule == "remote") allowed = false;
    else if (!isRemote && rule == "local") allowed = true;
    else if (rule[0] == "!" && rule.substring(1) === match) allowed = false;
  }
...
function inlineRemoteResource(importedFile, mediaQuery, context) {
  ...
  var proxyProtocol =
    context.inliner.request.protocol || context.inliner.request.hostname;
  var get =
  ...
  function handleError(message) {
    ...
    errorHandled = true;
    context.errors.push(
      'Broken @import declaration of "' + importedUrl + '" - ' + message
    );
  ...
  var requestOptions = override(
    url.parse(importedUrl),
...
function restoreImport(importedUrl, mediaQuery, context) {
  var restoredImport =
    ...
    importedUrl +
    ")" +
    (mediaQuery.length > 0 ? " " + mediaQuery : "") +
    ";";
