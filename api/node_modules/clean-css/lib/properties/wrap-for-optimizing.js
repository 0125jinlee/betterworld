...
function hackType(property) {
  ...
  if (name[0] == UNDERSCORE_HACK) {
    ...
    lastValue[0][0] == BANG_HACK &&
    !lastValue[0].match(IMPORTANT_WORD_MATCH)
  ) {
    type = "bang";
  } else if (
    lastValue[0].indexOf(BANG_HACK) > 0 &&
    !lastValue[0].match(IMPORTANT_WORD_MATCH)
  ) {
    type = "bang";
  } else if (
    lastValue[0].indexOf(BACKSLASH_HACK) > 0 &&
    lastValue[0].indexOf(BACKSLASH_HACK) ==
      lastValue[0].length - BACKSLASH_HACK.length - 1
  ) {
    type = "backslash";
  } else if (
    lastValue[0].indexOf(BACKSLASH_HACK) === 0 &&
    lastValue[0].length == 2
  ) {
    type = "backslash";
  }
...
function stripImportant(property) {
  if (property.length > 0)
    property[property.length - 1][0] = property[property.length - 1][0].replace(
      IMPORTANT_TOKEN_MATCH,
      ""
    );
}
...
function stripSuffixHack(property, hackType) {
  ...
  lastValue[0] = lastValue[0]
    .substring(
      0,
      lastValue[0].indexOf(hackType == "backslash" ? BACKSLASH_HACK : BANG_HACK)
    )
    .trim();
