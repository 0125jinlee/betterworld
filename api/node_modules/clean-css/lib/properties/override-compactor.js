...
function wouldResultInLongerValue(left, right) {
  ...
  if (left.multiplex) {
    component = multiplexClone.components.filter(
      nameMatchFilter(simpleClone)
    )[0];
    overrideIntoMultiplex(component, simpleClone);
  } else {
    component = simpleClone.components.filter(
      nameMatchFilter(multiplexClone)
    )[0];
  ...
  propertyLoop: for (i = properties.length - 1; i >= 0; i--) {
    ...
    for (j = i - 1; j >= 0; j--) {
      ...
      if (!left.shorthand && right.shorthand && isComponentOf(right, left)) {
        ...
        if (
          !anyValue(validator.isValidFunction, left) &&
        ...
        mayOverride =
          (compactable[left.name] && compactable[left.name].canOverride) ||
          canOverride.sameValue;
        ...
        if (everyCombination(mayOverride, component, right, validator)) {
          ...
          var nonMergeableValue =
            compactable[right.name].nonMergeableValue === right.value[0][0];
          ...
          if (
            component.value[0][0] != right.value[0][0] &&
        ...
        for (k = left.components.length - 1; k >= 0; k--) {
          ...
          mayOverride =
            compactable[leftComponent.name].canOverride ||
            canOverride.sameValue;
          if (
            !everyCombination(
              mayOverride,
          ...
          if (
            !everyCombination(
              canOverride.twoOptionalFunctions,
              leftComponent,
              rightComponent,
              validator
            ) &&
            validator.isValidFunction(rightComponent)
          )
            continue propertyLoop;
        }
        ...
        component = left.components.filter(nameMatchFilter(right))[0];
        mayOverride =
          compactable[right.name].canOverride || canOverride.sameValue;
        if (!everyCombination(mayOverride, component, right, validator))
          continue;
