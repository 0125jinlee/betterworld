...
function _styleFilter(validator) {
  return function(value) {
    return (
      value[0] != "inherit" &&
      validator.isValidStyle(value[0]) &&
      !validator.isValidColorValue(value[0])
    );
  };
}
...
function _widthFilter(validator) {
  return function(value) {
    return (
      value[0] != "inherit" &&
      validator.isValidWidth(value[0]) &&
      !validator.isValidStyleKeyword(value[0]) &&
      !validator.isValidColorValue(value[0])
    );
  };
}
...
function background(property, compactable, validator) {
  ...
  for (var i = values.length - 1; i >= 0; i--) {
    ...
    if (validator.isValidBackgroundAttachment(value[0])) {
      ...
      validator.isValidBackgroundPositionPart(value[0]) ||
      validator.isValidBackgroundSizePart(value[0])
    ) {
      if (i > 0) {
      ...
      (color.value[0][0] == compactable[color.name].defaultValue ||
        color.value[0][0] == "none") &&
      validator.isValidColor(value[0])
    ) {
      color.value = [value];
    } else if (
      validator.isValidUrl(value[0]) ||
      validator.isValidFunction(value[0])
    ) {
      image.value = [value];
    }
  }
...
function widthStyleColor(property, compactable, validator) {
  ...
  if (
    (property.value.length == 1 && property.value[0][0] == "inherit") ||
    (property.value.length == 3 &&
      property.value[0][0] == "inherit" &&
      property.value[1][0] == "inherit" &&
      property.value[2][0] == "inherit")
  ) {
  ...
  if (values.length > 0) {
    ...
    match =
      matches.length > 1 && (matches[0][0] == "none" || matches[0][0] == "auto")
        ? matches[1]
        : matches[0];
    if (match) {
      width.value = [match];
