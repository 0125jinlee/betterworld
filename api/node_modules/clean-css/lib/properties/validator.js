...
var cssUnitRegexStr =
  "(\\-?\\.?\\d+\\.?\\d*(" + allUnits.join("|") + "|)|auto|inherit)";
...
var cssFunctionAnyRegexStr =
  "(" +
  cssVariableRegexStr +
  "|" +
  cssFunctionNoVendorRegexStr +
  "|" +
  cssFunctionVendorRegexStr +
  ")";
var cssUnitOrCalcRegexStr = "(" + cssUnitRegexStr + "|" + cssCalcRegexStr + ")";
var cssUnitAnyRegexStr =
  "(none|" +
  widthKeywords.join("|") +
  "|" +
  cssUnitRegexStr +
...
var cssFunctionNoVendorRegex = new RegExp(
  "^" + cssFunctionNoVendorRegexStr + "$",
  "i"
);
var cssFunctionVendorRegex = new RegExp(
  "^" + cssFunctionVendorRegexStr + "$",
  "i"
);
...
function Validator(compatibility) {
  ...
  var compatibleCssUnitRegexStr =
    "(\\-?\\.?\\d+\\.?\\d*(" + validUnits.join("|") + "|)|auto|inherit)";
  this.compatibleCssUnitRegex = new RegExp(
    "^" + compatibleCssUnitRegexStr + "$",
    "i"
  );
  this.compatibleCssUnitAnyRegex = new RegExp(
    "^(none|" +
      widthKeywords.join("|") +
      "|" +
      compatibleCssUnitRegexStr +
...
Validator.prototype.isValidRgbaColor = function(s) {
  s = s.split(" ").join("");
  return (
    s.length > 0 && s.indexOf("rgba(") === 0 && s.indexOf(")") === s.length - 1
  );
};
...
Validator.prototype.isValidHslaColor = function(s) {
  s = s.split(" ").join("");
  return (
    s.length > 0 && s.indexOf("hsla(") === 0 && s.indexOf(")") === s.length - 1
  );
};
...
Validator.prototype.isValidNamedColor = function(s) {
  ...
  return (
    s !== "auto" &&
    (s === "transparent" || s === "inherit" || /^[a-zA-Z]+$/.test(s))
  );
};
...
Validator.prototype.isValidBackgroundAttachment = function(s) {
  return (
    backgroundAttachmentKeywords.indexOf(s) >= 0 || this.isValidVariable(s)
  );
};
...
Validator.prototype.isValidBackgroundPositionPart = function(s) {
  return (
    backgroundPositionKeywords.indexOf(s) >= 0 ||
    cssUnitOrCalcRegex.test(s) ||
    this.isValidVariable(s)
  );
};
...
Validator.prototype.isValidBackgroundPosition = function(s) {
  ...
  for (var i = 0, l = parts.length; i < l; i++) {
    if (parts[i] === "") continue;
    if (
      this.isValidBackgroundPositionPart(parts[i]) ||
      this.isValidVariable(parts[i])
    )
      continue;
...
Validator.prototype.isValidBackgroundSizePart = function(s) {
  return (
    backgroundSizeKeywords.indexOf(s) >= 0 ||
    cssUnitRegex.test(s) ||
    this.isValidVariable(s)
  );
};
...
Validator.prototype.isValidBackgroundPositionAndSize = function(s) {
  ...
  return (
    this.isValidBackgroundSizePart(twoParts.pop()) &&
    this.isValidBackgroundPositionPart(twoParts.pop())
  );
};
