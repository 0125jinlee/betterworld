...
function _optimize(properties, mergeAdjacent, aggressiveMerging, validator) {
  ...
  function sameValue(position) {
    ...
    return (
      stringifyProperty(left.all, left.position) ==
      stringifyProperty(right.all, right.position)
    );
  }

  propertyLoop: for (
    var position = 0, total = properties.length;
    position < total;
    position++
  ) {
    var property = properties[position];
    var _name =
      property.name == "-ms-filter" || property.name == "filter"
        ? lastName == "background" || lastName == "background-image"
          ? lastName
          : property.name
        : property.name;
    ...
    if (
      ...
      lastProperty &&
      _name == lastName &&
      isImportant == lastProperty.important &&
      isHack == lastProperty.hack &&
      sameValue(position) &&
      !lastProperty.unused
    ) {
    ...
    if (
      _name in overrideMapping &&
      ((aggressiveMerging && _name != lastName) || mergeablePosition(position))
    ) {
      ...
      for (j = toOverridePositions.length - 1; j >= 0; j--) {
        ...
        if (
          ...
          !longhandToShorthand &&
          canOverride &&
          !canOverride(toRemove, property, validator)
        )
          continue;
