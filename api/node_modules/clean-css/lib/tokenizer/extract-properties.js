...
function withoutComments(string, into, heading, context) {
  ...
  while (matcher.test(string)) {
    ...
    string =
      string.substring(0, startOfComment) + string.substring(endOfComment);
...
function extractProperties(string, selectors, context) {
  ...
  if (string.indexOf(")") > -1)
    string = string.replace(
      /\)([^\s_;:,\)])/g,
      context.sourceMap ? ") __ESCAPED_COMMENT_CLEAN_CSS(0,-1)__ $1" : ") $1"
    );

  if (string.indexOf("ESCAPED_URL_CLEAN_CSS") > -1)
    string = string.replace(
      /(ESCAPED_URL_CLEAN_CSS[^_]+?__)/g,
      context.sourceMap ? "$1 __ESCAPED_COMMENT_CLEAN_CSS(0,-1)__ " : "$1 "
    );
  ...
  for (var i = 0, l = candidates.length; i < l; i++) {
    ...
    if (isVariable && firstBraceAt > 0) {
      ...
      var blockContent = candidate.substring(
        firstBraceAt + 1,
        candidate.length - blockSuffix.length
      );
    ...
    var values = split(
      candidate.substring(firstColonAt + 1),
      valueSeparator,
      true
    );
    ...
    if (values.length == 1 && values[0] === "") {
      context.warnings.push(
        "Empty property '" +
          name +
          "' inside '" +
          selectors.filter(selectorName).join(",") +
          "' selector. Ignoring."
      );
      continue;
    }
    ...
    for (var j = 0, m = values.length; j < m; j++) {
      ...
      var endsWithNonSpaceSeparator =
        trimmed.length > 1 &&
        (lastCharacter == COMMA || lastCharacter == FORWARD_SLASH);
      ...
      if (
        IMPORTANT_TOKEN_MATCH.test(trimmed) ||
        (IMPORTANT_WORD_MATCH.test(trimmed) &&
          body[pos][0][body[pos][0].length - 1] == "!")
      ) {
        context.track(trimmed);
        body[pos][0] += trimmed;
        continue;
