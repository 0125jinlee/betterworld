  ...
  var context = {
    ...
    resolvePath: outerContext.options.explicitTarget
      ? relativePathResolver(
          outerContext.options.root,
          outerContext.options.target
        )
      : null,
    source: undefined,
...
function relativePathResolver(root, target) {
  ...
  return function(relativeTo, sourcePath) {
    return relativeTo != sourcePath
      ? path.normalize(
          path.join(
            path.relative(rebaseTo, path.dirname(relativeTo)),
            sourcePath
          )
        )
      : sourcePath;
...
function whatsNext(context) {
  ...
  if (mode == "body") {
    ...
    closest =
      context.cursor +
      split(chunk.substring(context.cursor - 1), "}", true, "{", "}")[0]
        .length -
      2;
  ...
  if (
    nextSpecial > -1 &&
    context.cursor > 0 &&
    !/\s|\{|\}|\/|_|,|;/.test(chunk.substring(nextSpecial - 1, nextSpecial))
  ) {
    nextSpecial = -1;
  }
...
function intoTokens(context) {
  ...
  while (true) {
      ...
      if (whatsLeft.trim().length > 0) {
        if (context.mode == "body") {
          context.warnings.push(
            "Missing '}' after '" + whatsLeft + "'. Ignoring."
          );
        } else {
    ...
    if (what == "special") {
      ...
      var isSingle =
        firstSemicolonAt > -1 &&
        (firstOpenBraceAt == -1 || firstSemicolonAt < firstOpenBraceAt);
      var isBroken = firstOpenBraceAt == -1 && firstSemicolonAt == -1;
      if (isBroken) {
        context.warnings.push(
          "Broken declaration: '" + chunk.substring(context.cursor) + "'."
        );
      ...
      var selectors = extractSelectors(
        chunk.substring(context.cursor, nextSpecial),
        context
      );
      ...
      if (context.mode == "top") {
        ...
        var warning =
          chunk[context.cursor] == "}"
            ? "Unexpected '}' in '" +
              chunk.substring(at - 20, at + 20) +
              "'. Ignoring."
            : "Unexpected content: '" +
              chunk.substring(at, nextSpecial + 1) +
              "'. Ignoring.";
