...
function byUrl(data, context, callback) {
  ...
  for (; nextEnd < data.length; ) {
    ...
    nextStartUpperCase = hasUppercaseUrl
      ? data.indexOf(UPPERCASE_URL_PREFIX, nextEnd)
      : -1;
    ...
    if (data[nextStart + URL_PREFIX.length] == '"') {
      ...
      isDataURI = DATA_URI_PREFIX_PATTERN.test(
        data.substring(nextStart + URL_PREFIX.length)
      );

      if (isDataURI) {
        firstMatch = split(
          data.substring(nextStart),
          DATA_URI_TRAILER_PATTERN,
          false,
          "(",
          ")",
          true
    ...
    if (nextEnd == -1) {
      ...
      if (nextEnd == -1) nextEnd = data.length;
      else nextEnd--;

      context.warnings.push(
        "Broken URL declaration: '" +
          data.substring(nextStart, nextEnd + 1) +
          "'."
      );
    } else {
...
function byImport(data, context, callback) {
  ...
  for (; nextEnd < data.length; ) {
    ...
    if (
      nextImport > -1 &&
      nextImportUpperCase > -1 &&
      nextImportUpperCase < nextImport
    )
    ...
    if (
      nextSingleQuote > -1 &&
      nextDoubleQuote > -1 &&
      nextSingleQuote < nextDoubleQuote
    ) {
    ...
    if (
      nextEnd == -1 ||
      /^@import\s+(url\(|__ESCAPED)/i.test(untilNextQuote) ||
      COMMENT_END_MARKER.test(untilNextQuote)
    ) {
      cursor = nextStart;
      break;
    }
