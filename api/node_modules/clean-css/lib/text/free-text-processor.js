...
function normalize(text, data, prefixContext, cursor) {
  ...
  if (prefixContext) {
    searchIn =
      prefixContext +
      data.substring(0, data.indexOf("__ESCAPED_FREE_TEXT_CLEAN_CSS"));
  ...
  else if (lastSemicolon == -1) lastOne = lastOpenBrace;
  else lastOne = lastSemicolon;
...
FreeTextProcessor.prototype.restore = function(data, prefixContext) {
  ...
  for (; cursor < data.length; ) {
    ...
    var text = normalize(
      this.matches.restore(nextMatch.match),
      data,
      prefixContext,
      nextMatch.start
    );
