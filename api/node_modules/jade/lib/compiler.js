...
Compiler.prototype = {
  ...
  prettyIndent: function(offset, newline) {
    ...
    if (this.parentIndents) this.buf.push("buf.push.apply(buf, jade_indent);");
  },
  ...
  visitMixinBlock: function(block) {
    if (this.pp)
      this.buf.push(
        "jade_indent.push('" + Array(this.indents + 1).join(this.pp) + "');"
      );
  ...
  visitMixin: function(mixin) {
    ...
    if (mixin.call) {
      ...
      if (pp)
        this.buf.push(
          "jade_indent.push('" + Array(this.indents + 1).join(pp) + "');"
        );
  ...
  visitTag: function(tag) {
    ...
    if (tag.selfClosing || (!this.xml && selfClosing[tag.name])) {
      ...
      if (
        ...
        throw errorAtNode(
          tag,
          new Error(name + " is self closing and should not have content.")
        );
      }
    } else {
    ...
    attrs.forEach(
      function(attr) {
        ...
        if (key === "class") {
          ...
          if (buffer) {
            this.buffer(
              runtime.attr(key, toConstant(attr.val), escaped, this.terse)
            );
          } else {
