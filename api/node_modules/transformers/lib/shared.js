...
function Transformer(obj) {
  ...
  if (this.isMinifier) minifiers[this.outputFormat] = this;
  else {
    var minifier = minifiers[this.outputFormat];
    if (minifier) {
      this.minify = function(str, options) {
        if (options && options.minify)
          return minifier.renderSync(
            str,
  ...
  for (var i = 0; i < this.engines.length; i++) {
    try {
      var res =
        this.engines[i] === "."
          ? null
          : (this.engine = require(this.engines[i]));
...
Transformer.prototype.renderSync = function(str, options) {
  ...
  if (this._renderSync) {
    return this.minify(
      this._renderSync(this.isBinary ? str : fixString(str), options),
      options
    );
  } else if (this.sudoSync) {
    options.sudoSync = true;
    var res, err;
    this._renderAsync(this.isBinary ? str : fixString(str), options, function(
      e,
      val
    ) {
      if (e) err = e;
    ...
    if (err) throw err;
      ...
      throw new Error(
        this.sudoSync.replace(/FILENAME/g, options.filename || "")
      );
    else
      throw new Error(
        "There was a problem transforming " +
          (options.filename || "") +
          " syncronously using " +
          this.name
      );
...
Transformer.prototype.render = function(str, options, cb) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (self._renderAsync) {
      self._renderAsync(
        self.isBinary ? str : fixString(str),
        clone(options),
        function(err, val) {
          if (err) reject(err);
...
Transformer.prototype.renderFile = function(path, options, cb) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (self._cache[path]) resolve(null);
    else
...
Transformer.prototype.renderFileSync = function(path, options) {
  ...
  return this.renderSync(
    this._cache[path] ? null : fs.readFileSync(path),
    options
  );
};
