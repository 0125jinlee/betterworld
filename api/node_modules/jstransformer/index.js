...
function normalizeFn(result) {
  if (typeof result === "function") {
    ...
    typeof result.fn === "function"
  ) {
    if ("dependencies" in result) {
      if (!Array.isArray(result.dependencies)) {
        throw new Error(
          "Result should have a dependencies property that is an array"
        );
      }
    } else {
      result.dependencies = [];
...
Transformer.prototype.compile = function(str, options) {
  if (!this.can("compile")) {
    if (this.can("compileAsync")) {
      ...
      throw new Error(
        'The Transform "' + this.name + '" does not support compilation'
      );
    }
  }
...
Transformer.prototype.compileFile = function(filename, options) {
  ...
  if (this._hasMethod("compileFile")) {
    ...
    return tr.normalizeFn(
      this._tr.compile(tr.readFileSync(filename, "utf8"), options)
    );
  }
};
Transformer.prototype.compileFileAsync = function(filename, options, cb) {
  ...
  if (this._hasMethod("compileFileAsync")) {
    return tr.normalizeFnAsync(
      this._tr.compileFileAsync(filename, options),
      cb
    );
    ...
    return tr.normalizeFnAsync(
      tr.readFile(filename, "utf8").then(
        function(str) {
          if (this._hasMethod("compileAsync")) {
...
Transformer.prototype.compileFileClient = function(filename, options) {
  ...
  if (this._hasMethod("compileFileClient")) {
    ...
    return tr.normalize(
      this._tr.compileClient(tr.readFileSync(filename, "utf8"), options)
    );
  }
};
Transformer.prototype.compileFileClientAsync = function(filename, options, cb) {
  ...
  if (this._hasMethod("compileFileClientAsync")) {
    return tr.normalizeAsync(
      this._tr.compileFileClientAsync(filename, options),
      cb
    );
...
Transformer.prototype.render = function(str, options, locals) {
  if (!this.can("render")) {
    if (this.can("renderAsync")) {
      ...
      throw new Error(
        'The Transform "' + this.name + '" does not support rendering'
      );
    }
  }
...
Transformer.prototype.renderAsync = function(str, options, locals, cb) {
  ...
  if (this._hasMethod("renderAsync")) {
    ...
    return tr.normalizeAsync(
      this.compileAsync(str, options).then(function(compiled) {
        return {
...
Transformer.prototype.renderFile = function(filename, options, locals) {
  if (typeof this._tr.renderFile === "function") {
    ...
    return tr.normalize(
      this._tr.render(tr.readFileSync(filename, "utf8"), options, locals)
    );
    ...
    return Promise.reject(
      new Error("This transform does not support synchronous rendering")
    );
  }
};
Transformer.prototype.renderFileAsync = function(
  filename,
  options,
  locals,
  cb
  ...
  if (typeof this._tr.renderFileAsync === "function") {
    return tr.normalizeAsync(
      this._tr.renderFileAsync(filename, options, locals),
      cb
    );
  } else if (typeof this._tr.renderFile === "function") {
    return tr.normalizeAsync(
      this._tr.renderFile(filename, options, locals),
      cb
    );
  } else if (
    this._hasMethod("compile") ||
    this._hasMethod("compileAsync") ||
    this._hasMethod("compileFile") ||
    this._hasMethod("compileFileAsync")
  ) {
    return tr.normalizeAsync(
      this.compileFileAsync(filename, options).then(function(compiled) {
        return {
          body: compiled.fn(options || locals),
          dependencies: compiled.dependencies
        };
